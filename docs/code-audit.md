...
- [ ] **(2026-06-18 — UNBLOCKED: Firestore rules now deployed + verified live)** `bootstrapPage()` **FULL** — also own the auth dispatch: `bootstrapPage({ auth: "public"|"user"|"admin"|"static", render })` so the unauth gate (currently copy-pasted 5×) and the requireAuth/requireAdmin guards have ONE definition. **Gate lifted 2026-06-18 — rules deployed + verified live.** The wait was for risk-containment: until the server enforced anything, the client gate was the *only* guard, so an auth-path bug introduced here would be live and unguarded. With server-side authz now live, the server backstops such mistakes, so this is the **next actionable refactor** — do it on its own branch (it touches every page's auth path). Prereqs recorded by the design workflow: add `export currentProfile()` to `auth.js`; normalize the callback to `{ main, user, profile }`; extract `event.html`'s ~150-line inline dispatch into `renderEvent(main, user)`; give `club.html` an `onGate` hook for its `_activeUser` signout reset; preserve onAuth-refires-vs-guard-resolves-once semantics. Chosen against now (per owner's inspectability bias — keep the gate visible per-page until centralization is worth the auth-path refactor risk).

...
- [ ] Decompose `admin.html` (1,966 inline lines) into per-tab controller modules under `js/admin/` (barrel like `js/ui.js`) — do last, incrementally. **Deferred from Wave 2** (CT event window; live-critical, no CI). **Full plan + ctx strategy + 9-step order + Step 1 recipe → [docs/admin-decomposition-plan.md](admin-decomposition-plan.md)**; work resumes on branch `admin-decomposition`. _(F-20)_

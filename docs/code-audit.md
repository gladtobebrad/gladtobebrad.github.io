...
> **Owner:** Blake · **Last reviewed:** 2026-06-18
...
- 2026-06-18 — **Wave 3 (nice-to-haves) — wrapped on branch `wave3-polish`.** Shipped the doable items: deterministic `computeFinishPositions` tiebreak (stable `wslId`, re-scrape-idempotent; guarded by a new `test/wsl-resolve.test.mjs` verified to fail against the old `Math.random()`); `--color-terracotta-dark` token; 48 `<th scope="col">` across 6 files; about.html OG/Twitter tags; the safe `var(--radius-lg, 12px)`→`var(--radius-lg)` fix; and a breakpoint **legend** (off-values are intentional — e.g. `1000px` keeps iPad-landscape 2-col — so documented, not collapsed). Bonus: fixed `test/security-helpers.test.mjs`, silently broken since the ui.js split (it grepped `js/ui.js` instead of `js/ui/escape.js`) → 17/17 again. **Paused:** the countdown DHMS/interval consolidation (not a clean extract; live-tick + fractional-tz risk) — proposal recorded in the Wave 3 item. Still open: the ad-hoc `6px`/`10px` radii (visual call) + the `signIn()`→return-to-originating-page follow-up. Done autonomously in one session; each item individually committed by the owner.
...
- 2026-06-18 — **Wave 2 FINAL chunk — decomposed the admin.html god-module (F-20):** carved the 1,966-line single-file admin panel into 9 focused controllers under `js/admin/` (`events` 303, `wsl-import` 287, `repricing` 234, `leaderboard` 219, `banners` 150, `surfers` 136, `results` 134, `players` 32, `clubs` 26) behind a 9-export `js/admin.js` **barrel** (mirroring the ui.js split). The shell collapses to a **532-line** orchestrator: imports + the 6 state `let`s + a `ctx()` factory (current-state snapshot + `reload`/`rerender`/`set` write-backs — the "Option B: explicit per-controller context" strategy) + the `renderAdmin` template + a ~32-line `wireAdminEvents` dispatcher that hands each tab `wireX(ctx())`. Shipped as **9 independently smoke-tested + committed steps** (per the [admin-decomposition-plan.md](admin-decomposition-plan.md) recipe). Every extraction kept **byte-verbatim except the documented `ctx` transforms** and adversarially diff-verified: slice-vs-`HEAD` `diff` == `diff -w` (zero whitespace/dedent drift), exact transform-count asserts, a dangling-reference audit (no leaked shell-only bindings), per-module import-usage check, and `node --check` across the whole tree. The integrity-critical `recalcLeaderboardForTour` moved byte-for-byte (`diff -w` clean). Steps 1–8 ran on branch `admin-decomposition`; the final events.js step went straight to `main` (the live CT window had passed; no CI). **Completes Wave 2.**
- 2026-06-18 — **Wave 0 security rules DEPLOYED + VERIFIED → the critical gap is closed.** The collaborator deployed the Firestore rules; Blake (now Firebase Editor, logged into the CLI as `blakecole000@gmail.com`) re-deployed both `firestore.rules` + `storage.rules` from the repo via `firebase deploy --only firestore:rules,storage` on project `fantasysurfer`. Both compiled + released; `firebase deploy` reported each as **"already up to date / skipping upload"**, confirming the live rulesets are **byte-identical to the repo** (no console drift) — and re-released Storage (whose console editor had appeared empty, most likely a render glitch rather than a missing ruleset). **Server-side authorization is now live**, closing the audit's one disqualifying gap (§1; theme A) and lifting the gate on the deferred `bootstrapPage()` **FULL** auth-dispatch refactor (§3). Still open (collaborator / GCP-Console only): Firebase API-key HTTP-referrer restriction + App Check.
- 2026-06-18 — **Auth-dispatch centralization — `bootstrapPage()` FULL (F-31, the last deferred Wave 2 structural item):** with server-side rules now live (the gate that had blocked it), folded the auth dispatch into `bootstrapPage({ auth, render, onUnauth, onGate })` in `js/ui/nav.js` — the unauth gate (was copy-pasted across 5 pages) + the requireAuth/requireAdmin guards now have ONE definition; added `currentProfile()` to `auth.js`. Modes: `static` (about), `user` inline-gate (standings/surfers/profile/club/index), `user`+`onUnauth:"redirect"` (event/team — redirect preserved, zero behavior change), `admin` (admin). render = `({ main, user, profile })`; "user" renders on a sign-in transition and skips no-op same-user refires (the one intentional change — a spurious token-refresh re-render nothing relied on; sidesteps team's roster-edit-stomp risk). Shipped on branch `auth-dispatch-refactor` in 9 incremental, individually smoke-tested + committed steps (foundation → about → 3 vanilla gate pages → club → index → admin → event → team → cleanup), bracketed by two workflows — a 9-agent page-mapper (before) and a 9-agent adversarial equivalence audit (after): **all 9 pages verdicted behavior-equivalent, zero concerns**. No page imports onAuth/requireAuth/requireAdmin/showAuthGate anymore; full `node --check` clean.
- 2026-06-18 — **Wave 2 chunk 8 — split the ui.js god-module (F-20):** carved the 909-line `js/ui.js` into 5 focused modules under `js/ui/` — `escape.js` (escapeHtml/safeUrl leaf), `format.js`, `banners.js`, `modals.js`, `nav.js` — with a clean acyclic graph (escape ← {format, banners, modals} ← nav). `js/ui.js` is now a 10-line flat re-export **barrel** (explicit named re-exports, not `export *`, so a future cross-module name collision is a hard error), leaving all 9 pages + `standings-table.js` untouched. Dynamic db/wsl-scrape/firebase imports localized to banners/nav and rewritten to `../` (F-26). Designed + verified via a 4-agent workflow (2 call-graph mappers → split design → adversarial acyclicity/coverage check — which refuted the suspected renderHeader↔openProfileEditModal cycle: on save it does `location.reload()`). Carved byte-identically by line-slice (assert-guarded: all 22 exports placed exactly once); 17/17 behavioral checks on the firebase-free modules + full `node --check`.
- 2026-06-18 — **Wave 2 chunk 7 — bootstrapPage() (F-31):** added `bootstrapPage()` to `ui.js` (runs `initAuth`/`renderHeader`/`renderFooter`, returns `#app-main`) and migrated all 9 pages — each drops the 3-line preamble + the `initAuth`/`renderHeader`/`renderFooter` imports + the redundant in-callback `main` lookup (incl. the secondary re-fetches in club/team), keeping its auth dispatch visible in-page. Shipped the **minimal** design (zero auth-path control-flow change) and deferred the **full** dispatch-centralizing version to the monetization milestone (see deferred item) — chosen via a 12-agent workflow: 9 parallel page-mappers → design (minimal vs full) → adversarial faithfulness verifier (confirmed minimal changes no observable auth behavior; timing safe since module scripts are deferred so `#app-main` exists when `bootstrapPage()` runs). All 10 files pass `node --check`.
- 2026-06-17 — **Wave 2 chunk 6 — tour-label helper (F-50):** added `tourLabel`/`tourLabelFull`/`tourAbbr` to `config.js` and routed all 14 dynamic ternary/local-helper sites + 4 JS-context paired-static call args through them; left pure-HTML static literals + about.html prose as content (inspectability > marginal single-source on always-one-tour markup). Resolved the `labelOf` dup and a `const tourLabel` shadowing collision. Cataloged + designed via a 5-agent discovery workflow (multi-modal finders → adversarial completeness critic → synthesis); 19/19 equivalence checks pass.
...
> **Update 2026-06-18 — this gap is CLOSED.** `firestore.rules` + `storage.rules` (authored in Wave 0) are **deployed and verified live** on project `fantasysurfer` — a repo-side `firebase deploy` reported both rulesets "already up to date" (content identical to the repo) and re-released them. Server-side authorization now exists; the original verdict above is preserved as the as-audited record.
...
- [x] **Authored** `firestore.rules` + `firebase.json` + `storage.rules`: `teams` writes locked to `uid` + `tradingOpen==true`, `isAdmin` client-immutable, `surfers/events/results/leaderboard/config/meta` admin-only, `avatars/{uid}` scoped. _(F-53, F-60, F-56, F-55)_ → **✅ deployed + verified live 2026-06-18** (collaborator deployed Firestore; Blake re-deployed Firestore + Storage from the repo via CLI — `firebase deploy` reported both rulesets "already up to date", confirming live == repo). ([firebase-deploy-handoff.md](firebase-deploy-handoff.md))
...
- [x] Lift label/name helpers into shared modules — `tourLabel`/`tourLabelFull`/`tourAbbr` → **`config.js`** (pure, co-located with `TOURS`; *not* ui.js, which imports auth.js) _(F-50)_; `splitName`+`nameLabelHtml` → `ui.js`; `avatarTile` → `standings-table.js`. `renderRankSparkline` **kept per-page** (dashboard chart is intentionally tall, club chart flat — only the `buildRankProgression` data is shared)
- [x] Add a `bootstrapPage()` helper for the 8-page preamble — **minimal version** (runs `initAuth`/`renderHeader`/`renderFooter`, returns `#app-main`); every page now drops those 3 imports/calls + the redundant `main` lookup and keeps its own visible auth dispatch. Centralizing the dispatch itself (the FULL version) is deliberately deferred — see escape-hatch below _(F-31)_
...
- [x] Split `ui.js` (909 lines) into `js/ui/{escape,format,banners,modals,nav}.js` (added a leaf `escape.js` the audit didn't call out); `js/ui.js` kept as a flat re-export **barrel** so all 10 consumers stay unchanged. Acyclic: escape ← {format, banners, modals} ← nav _(F-20, F-26)_
- [x] Decompose `admin.html` (1,966 inline lines) into per-tab/-service controller modules under `js/admin/` behind a `js/admin.js` barrel (like `js/ui.js`). **Done 2026-06-18** — 9 incremental, individually smoke-tested + committed steps. Shell down to a **532-line** orchestrator (imports + 6 state `let`s + `ctx()` factory + `renderAdmin` template + a ~32-line `wireAdminEvents` dispatcher); logic in 9 modules (`events` 303, `wsl-import` 287, `repricing` 234, `leaderboard` 219, `banners` 150, `surfers` 136, `results` 134, `players` 32, `clubs` 26). Each extraction byte-verbatim except the documented `ctx` transforms, adversarially diff-verified. **Plan/recipe → [docs/admin-decomposition-plan.md](admin-decomposition-plan.md)**. _(F-20)_

- [x] **(✅ DONE 2026-06-18)** `bootstrapPage()` **FULL** — now also owns the auth dispatch: `bootstrapPage({ auth: "static"|"user"|"admin", render, onUnauth, onGate })`, so the unauth gate (was copy-pasted 5×) and the requireAuth/requireAdmin guards have ONE definition in `js/ui/nav.js`. All 9 pages migrated on branch `auth-dispatch-refactor` (9 incremental, individually smoke-tested + committed steps), bracketed by two workflows — a 9-agent page-mapper (before) and a 9-agent adversarial equivalence audit (after, all 9 verdicted behavior-equivalent, zero concerns). Every recorded prereq landed: `currentProfile()` exported from `auth.js`; render normalized to `{ main, user, profile }`; event.html's ~150-line inline dispatch wrapped verbatim in the render callback (no separate `renderEvent` needed); club.html's `_activeUser` reset moved to the new `onGate` hook; once-vs-refire semantics preserved (redirect/admin resolve once; "user" gate renders on a sign-in transition and ignores no-op same-user refires — the lone intentional change, a spurious token-refresh re-render nothing relied on). Two judgement calls: dropped the speculative `"public"` mode (no consumer — index is just `"user"`), and event/team kept their redirect via `onUnauth:"redirect"` (zero behavior change, owner's pick over unifying to inline-gate).
...
- [x] **Deterministic tiebreak** in `computeFinishPositions` (2026-06-18) — `Math.random()` → stable `wslId` sort, so re-scrapes don't reshuffle tied surfers; exact ties still flagged. New `test/wsl-resolve.test.mjs` guards it (verified to fail against the old random code). _(wave3-polish)_
- [ ] **Consolidate countdown DHMS/timezone parsing + interval handles** — **investigated → PAUSED (2026-06-18).** Not a clean extract: the two DHMS *outputs* differ (banner `Nd : Nh : Nm : Ns` vs dashboard conditional-compact), and the two deadline parsers differ (banners.js handles fractional-hour offsets via `absM`; index.html's `getDeadlineMs` doesn't — unifying would change index's behavior). Touches the live public banner + dashboard + the global `window._countdownInterval`, so it needs a real-time tick smoke test. Proposed for a joint pass: export `parseDeadlineMs()` + `countdownParts(ms)→{d,h,m,s}` from `banners.js`; keep each caller's format + interval.
- [x] **`--color-terracotta-dark` token, `<th scope>`, OG tags on about.html, + the safe radius fix** (2026-06-18) — token added + wired (2 hover literals); 48 `<th scope="col">` across 6 files (`<thead>` + the aria-hidden scroll-spacer excluded); about.html got the full 10-tag OG/Twitter set; radius: only the dead `var(--radius-lg, 12px)` fallback fixed (resolves identically). *Left:* the ad-hoc `6px`/`10px` radii — changing them alters corner roundness (a visual call), and the audit itself warns against a spacing-system rewrite. _(wave3-polish)_
- [x] **Breakpoints — documented, NOT collapsed (2026-06-18).** On inspection the off-canonical values are load-bearing: the `1000px` dashboard break keeps the two-column layout at iPad-landscape (exactly 1024px); `900px`/`700px` are tablet tile-density / the surfers sticky-column trigger. Collapsing would regress, so added a **breakpoint legend** to `css/fantasy.css` documenting the canonical 1024/768/600/420 scale + why each off-value exists — which addresses the real "can't tell which are load-bearing" concern. _(wave3-polish)_
...
- **✅ Resolved (2026-06-18):** split into `js/ui/{escape,format,banners,modals,nav}.js` with `js/ui.js` retained as a flat re-export **barrel** — so, unlike the original "mechanical import-list edits" plan, ZERO consumers changed (lower risk on a no-CI site; direct per-page imports left as an optional later follow-up). Refinements vs the recommendation: (a) added a leaf `escape.js` (escapeHtml/safeUrl) since they're used by 4 of the 5 modules + standings-table.js — making them the graph root keeps every other module's deps minimal; (b) `openProfileEditModal` + `renderBanners` went to **nav.js** (not modals/banners) because `renderHeader` calls both — co-locating keeps those edges intra-module and the cross-module graph a strict DAG (escape ← {format, banners, modals} ← nav); (c) `showLoading`→format (trivial DOM string), `showAuthGate`→modals (it's an auth overlay). The db/wsl-scrape/firebase dynamic imports are localized to banners/nav (closes F-26's ask). Carved byte-identically (functions unchanged); all 22 exports re-exported; verified by a design+adversarial workflow, line-slice asserts, `node --check`, and 17 behavioral checks.
- **✅ Also resolved — admin.html (the other half of theme B, 2026-06-18):** the 1,966-line `admin.html` panel was decomposed into 9 controllers under `js/admin/` behind a `js/admin.js` barrel (same pattern), shrinking the shell to a 532-line orchestrator. Each tab/service controller takes an explicit `ctx()` (state snapshot + `reload`/`rerender`/`set` write-backs) rather than a file-wide rename, so `events`/`surfers` never collide with their string literals. Done in 9 individually-committed, byte-verbatim-except-`ctx`-transform steps, each adversarially diff-verified (slice-vs-`HEAD` `diff` == `diff -w`, transform-count asserts, dangling-reference audit, `node --check`). See the roadmap item + the 2026-06-18 progress-log entry.
...
- **✅ Resolved (minimal) 2026-06-18:** `bootstrapPage()` added to `ui.js` — runs `initAuth`/`renderHeader`/`renderFooter` and returns `#app-main`. All 9 pages migrated: each replaced the 3-line preamble with `const main = bootstrapPage();` (just `bootstrapPage();` for the static about.html), dropped `initAuth`/`renderHeader`/`renderFooter` from imports, and removed the now-redundant in-callback `document.getElementById("app-main")` (incl. the secondary re-fetches in club/team — `main` is now fetched exactly once). The recommended guard/render *centralization* was intentionally NOT folded in (kept the auth dispatch visible per-page); it's logged as a deferred item for the monetization milestone. `SEASON` was already centralized in Wave 1 (`js/config.js`). data.html no longer exists (deleted in Wave 2 chunk 1). A 12-agent design workflow (9 page-mappers → minimal-vs-full design → adversarial faithfulness check) verified the minimal migration changes no observable auth behavior.
...
- **✅ Resolved (2026-06-17):** Added `tourLabel`/`tourLabelFull`/`tourAbbr` to **`config.js`** (not ui.js — config.js is the pure leaf module, co-located with `TOURS`, importable by any layer without a cycle). Three small named functions instead of the proposed `{ short }` opts object (owner prefers named forms; `tourAbbr` earns its place via the single "W"/"M" site, admin.html:139). Routed all **14 dynamic sites** through them (admin.html ×13 incl. deleting the `labelOf` arrow and renaming the local `const tourLabel`→`tourName` collision; team.html ×1, folding a standalone `" CT"` into `tourLabelFull`) plus **4 JS-context paired-static** call args (`renderTeamCard`/`renderStandings` in index/profile). **Deliberately left** the pure-HTML static literals (tour-tab buttons, chart legends, `<option>`s, headings) and about.html prose as content — interpolating `${tourLabelFull("mens")}` into otherwise-static markup hurts inspectability for no behavioral gain (the tour is already encoded in the adjacent `data-tour`/`id`/`value`). Discovery + design done via a 5-agent workflow (3 multi-modal finders → adversarial completeness critic → synthesis); 19/19 equivalence checks confirm every replacement is a no-op.

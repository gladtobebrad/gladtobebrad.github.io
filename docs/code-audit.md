...
- [ ] **(Deferred 2026-06-18 → monetization milestone)** `bootstrapPage()` **FULL** — also own the auth dispatch: `bootstrapPage({ auth: "public"|"user"|"admin"|"static", render })` so the unauth gate (currently copy-pasted 5×) and the requireAuth/requireAdmin guards have ONE definition. Deferred because its only real win (a single canonical access check) earns its keep once there's something to charge for/protect — do it then, alongside the Wave 0 Firestore-rules work, so the client gate and server rules harden together. Prereqs recorded by the design workflow: add `export currentProfile()` to `auth.js`; normalize the callback to `{ main, user, profile }`; extract `event.html`'s ~150-line inline dispatch into `renderEvent(main, user)`; give `club.html` an `onGate` hook for its `_activeUser` signout reset; preserve onAuth-refires-vs-guard-resolves-once semantics. Chosen against now (per owner's inspectability bias — keep the gate visible per-page until centralization is worth the auth-path refactor risk).

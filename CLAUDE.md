...
1. **Anchor to season rank.** `fetchSeasonRankings(tour)` (in `wsl-scrape.js`) scrapes the official WSL rankings page (`/athletes/tour/{mct|wct}`) for fresh ranks. Each surfer's anchor comes from a nonlinear curve `value(rank) = RANKED_FLOOR + (peak − RANKED_FLOOR)·decay^(rank−1)` — elite ranks worth far more than mid-pack.
...
3. **Recency wiggle.** Each surfer is nudged ±up-to-$500K (`MAX_WIGGLE`) by `rank − finish` in the **most recent saved event** — pulled from Firestore results, not scraped.
...
Tunables live in `PRICING` (per-tour `peak` — the rank-#1 anchor, ~$11M, tuned so the top 5 sit ~$10M ±1M — plus `poolFactor`, `starters`, `cap`) in `pricing.js`. `RANKED_FLOOR = $3M` is the lowest price for a ranked surfer (the curve's asymptote); `WILDCARD_VALUE = $1.5M` is the wildcard price — the only value below the floor, so no surfer sits between $1.5M and $3M; `MAX_VALUE = $12.5M` the absolute ceiling (reserved for a top rank on a heater); and `MAX_CHANGE = $2M` the per-cycle movement cap. All values are multiples of $250K (enforced in the curve and in manual surfer-modal entry). The preview surfaces a tenability readout (top-N cost vs cap, affordable-stars, pool vs target) before Apply, which writes `value` + `rank` for matched surfers in one batch. There is no `priceBracket` field — it was removed as it had no gameplay effect.

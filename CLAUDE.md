...
Tunables in `pricing.js`: `PEAK` (the rank-#1 target / value ceiling, ~$11M, same both tours, tuned so the top 5 sit ~$10M ±1M) and per-tour `POOL_FACTOR` (the one genuinely per-tour knob); the salary `cap` and squad size `starters` come from `getTeamRules()` in `team.js`, not duplicated. Plus: `ALPHA = 0.5` (smoothing); `RANKED_FLOOR = $3M` (last-ranked surfer's price, the curve's pinned bottom); `WILDCARD_VALUE = $1.5M` (the only value below the floor — nothing between $1.5M and $3M); `MAX_CHANGE = $1.5M` (per-event backstop). The value ceiling is `PEAK` itself — there is no separate max constant; the curve tops out there and manual entry caps there too. All values are multiples of $250K (enforced in the curve and in manual surfer-modal entry, which also enforces the band and ceiling). The preview shows a tenability readout (top-N vs cap, affordable-stars, pool vs target) before Apply, which writes `value` + `rank` + `valuePrev` + `lastPricedEvent` for matched surfers in one batch. There is no `priceBracket` field — removed as it had no gameplay effect.
